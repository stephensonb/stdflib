    ...
    public class STDFReader : IDisposable, ISTDFReader
    {
        ...
        public virtual object Read(Type dataType, int dataLength)
        {
            return dataType.Name switch
            {
                ...
                "Char" => ReadChar(),
                "Char[]" => ReadString(dataLength < 0 ? ReadByte() : dataLength).ToCharArray(),
                "String" => ReadString(dataLength < 0 ? ReadByte() : dataLength),
                "BitField" => ReadBitField(),
                "BitField2" => ReadBitField2(),
                "Nibbles" => ReadNibbles(dataLength < 0 ? 1 : dataLength),
                "Byte[]" => ReadBytes(dataLength < 0 ? ReadByte() : dataLength),
                _ => null,
            };
        }
