...
namespace STDFLib2
{
    public class Surrogate<T> : ISurrogate, ISurrogate<T> where T : STDFRecord
    {
        ...
        {
            if (CurrentDefaults == null && CurrentObject.SupportsDefaults)
            {
        ...
        {
            if (CurrentObject.SupportsDefaults)
            {
                ulong hash = GetHashKey();
                if (hash != 0)
                {
        ...
        protected bool IsMissingValue(int index, object value) => IsEqual(value, CurrentInfo.GetMissingValue(index));
        protected bool IsMissingValue(string name, object value) => IsMissingValue(CurrentInfo.GetValueIndex(name), value);
