...
{
    /// <summary>
    /// Formatter that knows how to serialize/deserialize STDF formatted data to and from binary streams. 
    /// </summary>
    public class STDFBinaryFormatter : IFormatter, IDisposable
    {
        /// <summary>
        /// Base constructor for STDFBinaryFormatter.
        /// </summary>
        ...
        /// <summary>
        /// Deserializes an STDF object from the input stream.
        /// </summary>
        /// <param name="stream">Stream object to read data from.</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Serialize an STDF object to an output stream.
        /// </summary>
        /// <param name="stream">Stream object to write data to.</param>
        ...
        /// <summary>
        /// The type converter to use to convert Types to and from their respective binary representations.
        /// </summary>
        public IFormatterConverter Converter { get; set; }
        /// <summary>
        /// Indicates if the end of the input stream has been reached.
        /// </summary>
        ...
        protected VarData ReadVarData()
        {
            ...
            {
                GenericDataTypes.Byte => (VarData)ReadByte(),
                GenericDataTypes.UInt16 => (VarData)ReadUInt16(),
                GenericDataTypes.UInt32 => (VarData)ReadUInt32(),
                GenericDataTypes.SByte => (VarData)ReadSByte(),
                GenericDataTypes.Int16 => (VarData)ReadInt16(),
                GenericDataTypes.Int32 => (VarData)ReadInt32(),
                GenericDataTypes.Float => (VarData)ReadSingle(),
                GenericDataTypes.Double => (VarData)ReadDouble(),
                GenericDataTypes.String => (VarData)ReadString(),
                GenericDataTypes.ByteArray => (VarData)ReadByteArray(),
                GenericDataTypes.BitArray => (VarData)ReadBitArray(),
                GenericDataTypes.Nibble => (VarData)ReadByte(),
        ...
        protected void Write(byte[] buffer, int startIndex, int length)
        {
            SerializeStream.Write(buffer, startIndex, length);
        }
        ...
        protected void WriteArray(object obj, Type memberType, int startIndex, int length)
        {
            if (obj is Array values)
            {
                ...
                if (memberType.Name == "VarData")
                {
                    WriteVarDataArray((VarData[])obj, startIndex, length);
                }
                else
                {
        ...
        protected void WriteVarDataArray(VarData[] varData, int startIndex, int length)
        {
            ...
            for (int i = 0; i < varData.Length; i++)
            {
                ...
                WriteVarData(varData[i], byteOffset);
            }
        }
        protected void WriteBitArray(BitArray val)
        {
            Write(Converter.GetBytes(val, typeof(BitArray)));
        }
        protected void WriteByteArray(ByteArray val)
        {
            Write(Converter.GetBytes(val, typeof(ByteArray)));
        }
        protected void WriteNibbleArray(NibbleArray val)
        {
            Write(Converter.GetBytes(val, typeof(NibbleArray)));
        }
        protected void WriteDateTime(DateTime val)
        {
            Write(Converter.GetBytes(val, typeof(DateTime)));
        }
        protected void WriteVarData(VarData val, int byteOffset)
        {
        ...
        protected void WriteBoolean(bool val)
        {
            SerializeStream.WriteByte((byte)(val ? 1 : 0));
        }
        protected void WriteByte(byte val)
        {
            SerializeStream.WriteByte(val);
        }
        protected void WriteChar(char val)
        {
            Write(Converter.GetBytes(val));
        }
        protected void WriteDouble(double val)
        {
            Write(Converter.GetBytes(val));
        }
        protected void WriteInt16(short val)
        {
            Write(Converter.GetBytes(val));
        }
        ...
        protected void WriteSByte(sbyte val)
        {
            SerializeStream.WriteByte((byte)val);
        }
        protected void WriteSingle(float val)
        {
            Write(Converter.GetBytes(val));
        }
        ...
        protected void WriteUInt16(ushort val)
        {
            Write(Converter.GetBytes(val));
        }
        ...
        protected void WriteValueType(object obj, Type memberType)
        {
            Write(Converter.GetBytes(obj, memberType));
        }
